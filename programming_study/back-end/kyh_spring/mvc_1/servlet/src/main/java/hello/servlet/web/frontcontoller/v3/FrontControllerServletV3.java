...
public class FrontControllerServletV3 extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // paramMap
        Map<String, String> paramMap = createParamMap(request);  // 1. 파라미터는 Map 에 담아서 전달
        ModelView mv = controller.process(paramMap);  // 2. Model 은 따로 ModelView 로 전달
    ...
    private static Map<String, String> createParamMap(HttpServletRequest request) {
        ...
        request.getParameterNames().asIterator()
                .forEachRemaining(paramName -> paramMap.put(paramName, request.getParameter(paramName))); // request 에 들어있는 파라미터 이름과 값을 담는다.
        return paramMap;
    }
}